...
```{r}
plot_ly(data = mtcars,   x = ~cyl,  y = ~disp, type="scatter", mode = "markers+ text", text=~vs)
```
...
```
```{r}
fig <- ggplot2 :: ggplot ( data = mtcars, aes ( x = cyl, y = disp ) )  +
...
```{r}
plot_ly(gapminder_1952, x = ~gdpPercap, y = ~lifeExp, type="scatter", mode="markers") 
...
```{r}
gapminder_2007 %>%  plot_ly(x = ~gdpPercap, y = ~lifeExp, color = ~country, size=~pop) %>% layout(xaxis=list(type="log"))
```
...
```{r}
gapminder_2007 %>%  plot_ly(x = ~pop, y = ~lifeExp, color = ~continent) %>% layout(xaxis=list(type="log"))
```
...
```{r}
p1=ggplot(gapminder_1952,aes(x=gdpPercap,y=lifeExp))+
  geom_point()+
  scale_x_log10()+
...
plot_ly(by_continent, x = ~continent, y = ~meanLifeExp, type = 'bar', color= ~continent) %>% layout(title="Esperanza de vida promedio por continente ")
...
by_continent<- gapminder %>% 
 ...
 by_continent=  by_continent %>% arrange(desc(medianGdpPercap))
 by_continent
...
plot_ly(gapminder_2007, x = ~lifeExp, type = 'histogram', nbinsx=30)
...
```{r}
cat(paste("Mes: ", month(fecha, label = TRUE, abbr = FALSE),
